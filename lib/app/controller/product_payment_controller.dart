...
import 'package:user/app/backend/models/owner_info_model.dart';
import 'package:user/app/backend/models/owner_user_model.dart';
...
class ProductPaymentController extends GetxController implements GetxService {
  ...
  void onSelectAddress() {
    ...
    Get.toNamed(AppRouter.getAddressList(), arguments: ['product', selectedAddressId]);
  }
  ...
  void calculateDistance() {
    if (ownerType == 'individual') {
      ...
      debugPrint('-------');
      if (addressInfo.lat != null && addressInfo.lng != null && ownerInfo.lat != null && ownerInfo.lng != null) {
        ...
        if (distance > parser.getAllowedDeliveryRadius()) {
          ...
          showToast('${'Sorry we deliver the order near to'.tr} ${parser.getAllowedDeliveryRadius()}KM');
        } else {
      ...
      debugPrint('-------');
      if (addressInfo.lat != null && addressInfo.lng != null && salonInfo.lat != null && salonInfo.lng != null) {
        debugPrint('have address');
        double storeDistance = 0.0;
        double totalMeters = 0.0;
        storeDistance = Geolocator.distanceBetween(
  ...
  void onCoupon(String offerId, String offerName) {
    ...
    Get.toNamed(AppRouter.getCouponRoutes(), arguments: ['product', offerId, offerName]);
  }
  ...
  void onCheckout() {
    if (paymentId == 1) {
      ...
      Get.toNamed(AppRouter.getWebProductPayment(), arguments: [payMethodName, paymentURL]);
    } else if (paymentId == 3) {
      Get.delete<WebProductPaymentController>(force: true);
  ...
  Future<void> createOrder() async {
    ...
    var param = {
      "uid": parser.getUID(),
      "freelancer_id": ownerType == 'individual' ? Get.find<ProductCartController>().savedInCart[0].freelacerId : 0,
      "salon_id": ownerType == 'salon' ? Get.find<ProductCartController>().savedInCart[0].freelacerId : 0,
      ...
      "coupon_code": selectedCoupon.code != null ? jsonEncode(selectedCoupon) : 'NA',
      "extra": 'NA',
      "pay_key": 'COD',
      "status": 0,
      "payStatus": 1,
            ...
            children: [
              Container(
                ...
                child: ClipRRect(
                  ...
                  child: Image.asset('assets/images/sure.gif', height: 60, width: 60, fit: BoxFit.cover),
                ),
              ),
              ...
              ElevatedButton(
                onPressed: () {
                  // backOrders();
                  debugPrint('Product Payment On Orders Direct');
              ...
              TextButton(
                onPressed: () {
                  // backHome();
                  debugPrint('Product Payment On Home Direct');
                  Get.find<ProductCartController>().clearCart();
  ...
  Future<void> payWithInstaMojo() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (body['payment_request'] != '' && body['payment_request']['longurl'] != '') {
        ...
        Get.toNamed(AppRouter.getWebPayment(), arguments: [payMethodName, paymentURL]);
      }
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  // void backHome() {
  //   debugPrint('Product Payment On Home');
  //   Get.find<ProductCartController>().clearCart();
  ...
  // void backOrders() {
  //   debugPrint('Product Payment On Orders');
  //   Get.find<ProductCartController>().clearCart();
  //   Get.offAllNamed(AppRouter.getTabsBarRoute());
  //   Get.find<TabsController>().updateTabId(4);
  // }
