...
class RegisterController extends GetxController with GetSingleTickerProviderStateMixin {
  ...
  Future<void> onRegister() async {
    ...
    if (verificationMethod == 0) {
      ...
      if (smsName == '2') {
        var param = {'email': emailTextEditor.text, 'country_code': countryCode, 'mobile': mobileTextEditor.text};
        Get.dialog(
          SimpleDialog(
            children: [
              Row(
        ...
        if (response.statusCode == 200) {
          ...
          if (myMap['data'] != '' && myMap['data'] == true) {
            ...
            Get.toNamed(AppRouter.getFirebaseAuthRoutes(), arguments: [countryCode, mobileTextEditor.text, 'register']);
          } else {
            if (myMap['data'] != '' && myMap['data'] == false && myMap['status'] == 500) {
              showToast(myMap['message'.tr]);
            } else {
        ...
        var param = {'country_code': countryCode, 'mobile': mobileTextEditor.text, 'email': emailTextEditor.text};
        Get.dialog(
          SimpleDialog(
            children: [
              Row(
        ...
        if (response.statusCode == 200) {
          ...
          if (myMap['data'] != '' && myMap['data'] == true) {
            smsId = myMap['otp_id'];
            FocusManager.instance.primaryFocus?.unfocus();
            openOTPModal(countryCode.toString() + mobileTextEditor.text.toString());
          } else {
            if (myMap['data'] != '' && myMap['data'] == false && myMap['status'] == 500) {
              showToast(myMap['message'.tr]);
            } else {
  ...
  Future<void> createAccount() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (referralCodeTextEditor.text.isNotEmpty && referralCodeTextEditor.text != '') {
                  ...
                  children: [
                    ...
                    Expanded(
                      child: ElevatedButton(
                        ...
                        child: Text(
                          'Ok'.tr,
                          style: const TextStyle(color: ThemeProvider.whiteColor, fontSize: 16),
                        ),
                      ),
                    ),
                  ],
