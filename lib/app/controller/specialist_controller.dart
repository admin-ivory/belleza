...
import 'package:user/app/backend/models/packages_model.dart';
import 'package:user/app/backend/models/owner_reviews_model.dart';
import 'package:user/app/backend/models/userinfo_model.dart';
...
class SpecialistController extends GetxController with GetTickerProviderStateMixin implements GetxService {
  ...
  Future<void> getindividualDetails() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (individualDetails.images != 'NA' && individualDetails.images != null && individualDetails.images != '') {
  ...
  void onServicesView(int id, String name) {
    ...
    Get.toNamed(AppRouter.getIndividualList(), arguments: [id, name, individualDetails.uid]);
  }
  ...
  onMapCreated() {
    markers.add(Marker(markerId: const MarkerId('Id-1'), position: LatLng(individualDetails.lat as double, individualDetails.lng as double)));
  }
  ...
  Future<void> openWebsite() async {
    if (individualDetails.website.toString() != 'NA' || individualDetails.website!.isEmpty || individualDetails.website != null) {
...
//  }
  ...
  void onChat() {
    ...
    if (parser.isLogin() == true) {
      ...
      Get.toNamed(AppRouter.getChatRoutes(), arguments: [individualDetails.uid.toString(), '${userInfo.firstName} ${userInfo.lastName}']);
    } else {
