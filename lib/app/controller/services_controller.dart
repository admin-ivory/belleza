...
class ServicesController extends GetxController with GetTickerProviderStateMixin implements GetxService {
  ...
  Future<void> getSalonDetails() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (salonDetails.images != 'NA' && salonDetails.images != null && salonDetails.images != '') {
  ...
  onMapCreated() {
    markers.add(Marker(markerId: const MarkerId('Id-1'), position: LatLng(salonDetails.lat as double, salonDetails.lng as double)));
  }

  void onServicesView(int id, String name) {
    Get.delete<SelectedServicesController>(force: true);
    Get.toNamed(AppRouter.getSelectedServicesRoutes(), arguments: [id, name, salonDetails.uid]);
  }
  ...
  Future<void> openWebsite() async {
    if (salonDetails.website.toString() != 'NA' || salonDetails.website!.isEmpty || salonDetails.website != null) {
  ...
  Future<void> callSalon() async {
    final Uri launchUri = Uri(scheme: 'tel', path: salonDetails.mobile.toString());
  ...
  void onChat() {
    ...
    if (parser.isLogin() == true) {
      ...
      Get.toNamed(AppRouter.getChatRoutes(), arguments: [salonDetails.uid.toString(), salonDetails.name.toString()]);
    } else {
